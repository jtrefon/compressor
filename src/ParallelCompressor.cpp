...
std::vector<uint8_t>
ParallelCompressor::compress(const std::vector<uint8_t> &data) {
  ...
  for (std::size_t i = 0; i < chunkCount; ++i) {
    ...
    offset += sz;
    futures[i] =
        pool.enqueue([algoId = algoId_, c = std::move(chunk)]() mutable {
...
std::vector<uint8_t>
ParallelCompressor::decompress(const std::vector<uint8_t> &data) {
