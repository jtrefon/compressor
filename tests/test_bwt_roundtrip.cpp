...
static bool run_case(size_t n){
    ...
    compression::BwtCompressor bwt;
    auto c = bwt.compress(data);
    auto d = bwt.decompress(c);
    if (d != data){
        std::cerr << "BWT mismatch at size=" << data.size() << std::endl;
        return false;
    }
    return true;
}
...
int main(){
    try{
        ...
        if(!run_case(50000)) return 1;
        if(!run_case(100000)) return 1;
        if(!run_case(1000000)) return 1;
        if(!run_case(6000000)) return 1;
    } catch (const std::exception& e){ std::cerr << e.what() << std::endl; return 1; }
    std::cout << "BWT roundtrip tests: OK" << std::endl;
    return 0;
}
