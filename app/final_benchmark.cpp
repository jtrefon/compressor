...
// Include only the best working compressors
...
int main() {
    ...
    std::cout << "Starting FINAL benchmark using file: " << dataFilePath << std::endl;
    std::cout << "Read " << originalData.size() << " bytes." << std::endl;

    // --- Run Benchmarks ---
    std::vector<BenchmarkResult> results;
    
    // Default and curated set
    ...
    results.push_back(runBenchmark("BWT", std::make_unique<compression::BwtCompressor>(), originalData));
    results.push_back(runBenchmark("Ultra", std::make_unique<compression::UltraCompressor>(), originalData));
    results.push_back(runBenchmark("Extreme", std::make_unique<compression::ExtremeCompressor>(), originalData));
    ...
    // Summary
    std::cout << "\n🏆 SUMMARY:" << std::endl;
    std::cout << "Best compression: " << results[0].algorithmName << " (" << (results[0].ratio * 100.0) << "%)" << std::endl;
    std::cout << "Fastest compression: " << std::min_element(results.begin(), results.end(),
        [](const BenchmarkResult& a, const BenchmarkResult& b) {
            return a.compressionTimeMs < b.compressionTimeMs;
        })->algorithmName << std::endl;
    std::cout << "Fastest decompression: " << std::min_element(results.begin(), results.end(),
        [](const BenchmarkResult& a, const BenchmarkResult& b) {
            return a.decompressionTimeMs < b.decompressionTimeMs;
        })->algorithmName << std::endl;
